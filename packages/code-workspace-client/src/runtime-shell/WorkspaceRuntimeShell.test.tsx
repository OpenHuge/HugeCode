...
import { WorkspaceRuntimeShell } from "./WorkspaceRuntimeShell";
import { WorkspaceClientBindingsProvider } from "../workspace/WorkspaceClientBindingsProvider";

vi.mock("../workspace-app/WorkspaceApp", () => ({
  WorkspaceApp: () => <div data-testid="shared-workspace-app">Shared workspace app</div>,
}));
...
function createBindings(): WorkspaceClientBindings {
  return {
    ...
    platformUi: {
      ...
      renderWorkspaceHost: (children) => <div data-testid="workspace-host">{children}</div>,
...
describe("WorkspaceRuntimeShell", () => {
  it("renders the shared workspace app inside the host renderer", () => {
    render(
    ...
    expect(screen.getByTestId("workspace-host")).toBeTruthy();
    expect(screen.getByTestId("shared-workspace-app")).toBeTruthy();
  });
});
