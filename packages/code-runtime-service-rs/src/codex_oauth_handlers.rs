use super::*;
pub(super) use crate::codex_oauth_html::build_oauth_result_html;
...
pub(super) async fn codex_oauth_callback_handler(
    ...
    let local_codex_auth_path =
        resolve_local_codex_auth_path().map(|path| path.to_string_lossy().to_string());
    let local_codex_config_path =
        resolve_local_codex_config_path().map(|path| path.to_string_lossy().to_string());
    let local_codex_home =
        resolve_local_codex_home_dir().map(|path| path.to_string_lossy().to_string());
    let local_auth_persist_result = persist_local_codex_cli_auth_updates(
        token_payload.id_token.as_deref(),
        token_payload.access_token.as_deref(),
        token_payload.refresh_token.as_deref(),
        Some(api_credential.as_str()),
        parsed_claims.external_account_id.as_deref(),
        Some("chatgpt"),
    );
    if let Err(error) = local_auth_persist_result.as_ref() {
        warn!(
            error = error.as_str(),
            workspace_id = pending_login.workspace_id.as_str(),
            "failed to persist codex oauth tokens to local codex auth.json"
        );
    }
    ...
    metadata.insert(
        "localCodexAuthJsonSaved".to_string(),
        Value::Bool(local_auth_persist_result.is_ok()),
    );
    if let Some(path) = local_codex_auth_path.as_deref() {
        metadata.insert(
            "localCodexAuthJsonPath".to_string(),
            Value::String(path.to_string()),
        );
    }
    if let Some(path) = local_codex_config_path.as_deref() {
        metadata.insert(
            "localCodexConfigPath".to_string(),
            Value::String(path.to_string()),
        );
    }
    if let Some(path) = local_codex_home.as_deref() {
        metadata.insert(
            "localCodexHome".to_string(),
            Value::String(path.to_string()),
        );
    }
    if let Err(error) = local_auth_persist_result.as_ref() {
        metadata.insert(
            "localCodexAuthJsonError".to_string(),
    ...
    let response = (
        ...
        build_oauth_result_html(
            ...
            local_codex_auth_path
                .as_deref()
                .filter(|_| local_auth_persist_result.is_ok())
                .map(|path| format!("Account is now available in this workspace. Local Codex auth.json was saved to {path}."))
                .unwrap_or_else(|| {
                    local_auth_persist_result
                        .as_ref()
                        .err()
                        .map(|error| format!("Account is now available in this workspace, but local Codex auth.json could not be saved: {error}"))
                        .unwrap_or_else(|| "Account is now available in this workspace.".to_string())
                })
                .as_str(),
            true,
