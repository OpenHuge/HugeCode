...
include!("rpc_dispatch_task_sources_github.rs");
...
struct TaskSourceRecordRpc {
    ...
    github_source: Option<TaskSourceGitHubProvenanceRpc>,
    created_at: u64,
    payload: Value,
}
...
pub(crate) async fn handle_task_source_ingest_v1(
        ...
        if let Some((workspace_id, workspace_path)) = workspace_match.clone() {
                ...
                (
                    ...
                    TaskSourceLaunchOutcome {
                        ...
                        handshake: TaskSourceLaunchHandshakeRpc {
                            ...
                            summary: "Task source was ingested without requesting the runtime launch handshake."
                                .to_string(),
                            disposition: Some("not_requested".to_string()),
                            prepared_plan_version: None,
            ...
            (
                ...
                TaskSourceLaunchOutcome {
                    ...
                    handshake: TaskSourceLaunchHandshakeRpc {
                        state: "blocked".to_string(),
                        summary: "GitHub source intake could not resolve a connected workspace, so runtime launch did not begin."
                            .to_string(),
    ...
    let github_source = build_task_source_github_provenance(
        source_record_id.as_str(),
        &request.event,
        &request.payload,
        &launch_outcome.handshake,
    );
    ...
    let record = TaskSourceRecordRpc {
        ...
        github_source: Some(github_source),
        created_at,
...
async fn dispatch_task_source_launch(
    ...
    if let Some(existing_run_id) = find_active_run_for_source(ctx, workspace_id, payload).await {
            ...
            .is_some_and(|value| matches!(value, "continue" | "retry"))
        {
            ...
            .await?;
            let handshake = TaskSourceLaunchHandshakeRpc {
                state: "intervened".to_string(),
                summary:
                    "Runtime attached this GitHub follow-up to the active run instead of starting a new source-driven run."
                        .to_string(),
                disposition: Some("intervened".to_string()),
                prepared_plan_version: None,
                approved_plan_version: None,
            };
    ...
    let task_source = json!({
        ...
        "githubSource": build_task_source_github_provenance(
            source_record_id,
            event,
            payload,
            &TaskSourceLaunchHandshakeRpc {
                state: "prepared".to_string(),
                summary: "Runtime prepared a deterministic source-launch handshake before starting the GitHub-driven run."
                    .to_string(),
    ...
    let PreparedTaskSourceStartRequest {
        start_request,
        approved_plan_version,
    } = prepare_task_source_start_request(ctx, start_request).await?;
    ...
    let handshake = TaskSourceLaunchHandshakeRpc {
        ...
        summary: format!(
            "Runtime prepared plan version `{approved_plan_version}` and started the GitHub-driven run with the approved handshake."
        ),
        disposition: Some("launched".to_string()),
        prepared_plan_version: Some(approved_plan_version.clone()),
        approved_plan_version: Some(approved_plan_version.clone()),
    };
...
struct PreparedTaskSourceStartRequest {
    start_request: Value,
    approved_plan_version: String,
}

async fn prepare_task_source_start_request(
    ctx: &AppContext,
    mut start_request: Value,
) -> Result<PreparedTaskSourceStartRequest, RpcError> {
    let prepare =
    ...
    if let Some(task_source) = start_request.get_mut("taskSource") {
        task_source["githubSource"]["launchHandshake"] = serde_json::to_value(
            TaskSourceLaunchHandshakeRpc {
                state: "started".to_string(),
                summary: format!(
                    "Runtime prepared plan version `{approved_plan_version}` and promoted the approved plan into the GitHub source launch."
                ),
                disposition: Some("launched".to_string()),
                prepared_plan_version: Some(approved_plan_version.to_string()),
                approved_plan_version: Some(approved_plan_version.to_string()),
            },
        )
        .map_err(|error| RpcError::internal(format!("serialize launch handshake failed: {error}")))?;
    }
    Ok(PreparedTaskSourceStartRequest {
        start_request,
        approved_plan_version: approved_plan_version.to_string(),
    })
}
...
#[cfg(test)]
include!("rpc_dispatch_task_sources_tests.rs");
