  ...
  it("prepares, starts, and refreshes a governed GitHub launch in order", async () => {
    const preparation: RuntimeRunPrepareV2Response = {
      ...
      contextTruth: {
        ...
        ownerSummary: "operator",
        sourceMetadata: [],
        consumers: ["run"],
      },
      guidanceStack: {
        summary: "launch guidance",
        precedence: ["source", "launch"],
        layers: [],
      },
      triageSummary: {
        owner: "operator",
      ...
      delegationContract: {
        summary: "operator launches and agent executes",
        state: "launch_ready",
        humanOwner: "operator",
        agentExecutor: "runtime",
        accountability: "operator retains approval control",
        nextOperatorAction: "start",
        continueVia: null,
      },
      executionGraph: {
        graphId: "graph-1",
