...
describe("useThreadMessaging telemetry", () => {
  ...
  it("blocks pair and delegate sends when the selected model does not support tool use", async () => {
    const pushThreadErrorMessage = vi.fn();
    const markProcessing = vi.fn();
    ...
    const { result } = renderHook(() =>
      useThreadMessaging({
        ...
        effort: null,
        collaborationMode: null,
        toolsCapabilitySupport: "unsupported",
        missionMode: "pair",
        reviewDeliveryMode: "inline",
        steerEnabled: false,
        customPrompts: [],
    ...
    await act(async () => {
      await result.current.sendUserMessageToThread(workspace, "thread-1", "help me refactor", []);
    });
