...
function normalizeAccountSnapshot(response: Record<string, unknown> | null): AccountSnapshot {
  ...
  const providerRaw = typeof account.provider === "string" ? account.provider.trim() : "";
  const accountIdRaw = typeof account.accountId === "string" ? account.accountId.trim() : "";
  const externalAccountIdRaw =
    typeof account.externalAccountId === "string" ? account.externalAccountId.trim() : "";
  const displayNameRaw = typeof account.displayName === "string" ? account.displayName.trim() : "";
  const defaultChatgptWorkspaceTitleRaw =
    typeof account.defaultChatgptWorkspaceTitle === "string"
      ? account.defaultChatgptWorkspaceTitle.trim()
      : "";
  const authModeRaw = typeof account.authMode === "string" ? account.authMode.trim() : "";
  const localCliManagedRaw =
    typeof account.localCliManaged === "boolean" ? account.localCliManaged : null;
  ...
  return {
    ...
    ...(externalAccountIdRaw ? { externalAccountId: externalAccountIdRaw } : {}),
    ...(displayNameRaw ? { displayName: displayNameRaw } : {}),
    ...(defaultChatgptWorkspaceTitleRaw
      ? { defaultChatgptWorkspaceTitle: defaultChatgptWorkspaceTitleRaw }
      : {}),
    ...(authModeRaw ? { authMode: authModeRaw } : {}),
    ...(localCliManagedRaw !== null ? { localCliManaged: localCliManagedRaw } : {}),
  };
}
