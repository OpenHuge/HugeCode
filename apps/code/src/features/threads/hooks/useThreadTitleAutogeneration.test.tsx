...
describe("useThreadTitleAutogeneration", () => {
  ...
  it("still generates a title when the runtime placeholder is New thread", async () => {
    vi.mocked(generateRunMetadata).mockResolvedValue({
      title: "Generated Title",
      worktreeName: "feat/generated-title",
    });
    const { result, renameThread } = setup({ threadName: "New thread" });

    await act(async () => {
      await result.current.onUserMessageCreated("ws-1", "thread-1", "Hello from workspace");
    });

    expect(generateRunMetadata).toHaveBeenCalledWith("ws-1", "Hello from workspace");
    expect(renameThread).toHaveBeenCalledWith("ws-1", "thread-1", "Generated Title");
  });
