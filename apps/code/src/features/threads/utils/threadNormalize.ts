...
import { normalizeDistributedTaskGraphSnapshot } from "../../plan/types/distributedGraph";
...
function normalizeCreditsSnapshot(
  ...
  return {
    hasCredits:
      typeof hasCreditsRaw === "boolean" ? hasCreditsRaw : (previousCredits?.hasCredits ?? false),
    unlimited:
...
export function normalizeRateLimits(
  ...
  const planTypeValue =
    typeof raw.planType === "string"
      ...
      : typeof raw.plan_type === "string"
        ? raw.plan_type
        : null;
  const hasLimitIdKey = hasOwn(raw, "limitId") || hasOwn(raw, "limit_id");
  const limitIdRaw =
    typeof raw.limitId === "string"
      ? raw.limitId
      : typeof raw.limit_id === "string"
        ? raw.limit_id
        : null;
  const limitIdValue = limitIdRaw?.trim() ? limitIdRaw.trim() : null;
  const limitId = limitIdValue ?? (hasLimitIdKey ? null : (previous?.limitId ?? null));
  const includeLimitId = hasLimitIdKey || limitIdValue !== null || previous?.limitId !== undefined;
  const hasLimitNameKey = hasOwn(raw, "limitName") || hasOwn(raw, "limit_name");
  const limitNameRaw =
    typeof raw.limitName === "string"
      ? raw.limitName
      : typeof raw.limit_name === "string"
        ? raw.limit_name
        : null;
  const limitNameValue = limitNameRaw?.trim() ? limitNameRaw.trim() : null;
  const limitName = limitNameValue ?? (hasLimitNameKey ? null : (previous?.limitName ?? null));
  const includeLimitName =
    hasLimitNameKey || limitNameValue !== null || previous?.limitName !== undefined;
  ...
  return {
    ...
    ...(includeLimitId ? { limitId } : {}),
    ...(includeLimitName ? { limitName } : {}),
  };
}
