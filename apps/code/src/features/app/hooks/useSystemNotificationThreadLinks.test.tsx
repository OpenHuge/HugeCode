...
describe("useSystemNotificationThreadLinks", () => {
  it("navigates to the thread when the app regains focus", async () => {
    ...
    expect(setActiveTab).toHaveBeenCalledWith("missions");
  ...
  it("opens review detail for a pending runtime mission target when the app regains focus", async () => {
    const workspace = makeWorkspace({ connected: true });
    const workspacesById = new Map([[workspace.id, workspace]]);
    ...
    act(() => {
      result.current.recordPendingMissionTarget({
        kind: "mission",
        workspaceId: "ws-1",
        taskId: "task-1",
        runId: "run-1",
