...
export const REMOTE_THREAD_POLL_INTERVAL_MS = 12_000;
  ...
  useEffect(() => {
    ...
    const ensureWorkspaceConnected = () => {
      ...
      return Promise.resolve(reconnectWorkspaceRef.current(activeWorkspaceRef.current))
        .catch(() => {
          // Ignore reconnect failures so lifecycle refresh does not surface toast noise.
        })
        .finally(() => {
    ...
    const runRefresh = () => {
      ...
      void (async () => {
        ...
        .catch(() => {
          // Ignore refresh failures so lifecycle refresh does not surface toast noise.
        })
        .finally(() => {
