...
describe("useComposerImageDrop", () => {
  ...
  it("uses file paths on drop when available", async () => {
    ...
    const documentFile = new File(["data"], "notes.txt", { type: "text/plain" });
    (documentFile as File & { path?: string }).path = "/tmp/notes.txt";

    await act(async () => {
      await hook.result.handleDrop({
        dataTransfer: { files: [file, documentFile], items: [] },
        preventDefault: vi.fn(),
      } as unknown as React.DragEvent<HTMLElement>);
    });

    expect(onAttachImages).toHaveBeenCalledWith(["/tmp/photo.png", "/tmp/notes.txt"]);
