/** @vitest-environment jsdom */
import React, { act } from "react";
import { createRoot } from "react-dom/client";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { pickAttachmentFiles } from "../../../application/runtime/ports/desktopFiles";
import { useComposerImages } from "./useComposerImages";

vi.mock("../../../application/runtime/ports/desktopFiles", () => ({
  pickAttachmentFiles: vi.fn().mockResolvedValue([]),
}));

type HookResult = ReturnType<typeof useComposerImages>;

type RenderedHook = {
  result: HookResult;
  rerender: (next: { activeThreadId: string | null; activeWorkspaceId: string | null }) => void;
  unmount: () => void;
};

function renderComposerImages(initial: {
  activeThreadId: string | null;
  activeWorkspaceId: string | null;
}): RenderedHook {
  let props = initial;
  let result: HookResult | undefined;

  function Test() {
    result = useComposerImages(props);
    return null;
  }

  const container = document.createElement("div");
  document.body.appendChild(container);
  const root = createRoot(container);

  act(() => {
    root.render(React.createElement(Test));
  });

  return {
    get result() {
      if (!result) {
        throw new Error("Hook not rendered");
      }
      return result;
    },
    rerender: (next) => {
      props = next;
      act(() => {
        root.render(React.createElement(Test));
      });
    },
    unmount: () => {
      act(() => {
        root.unmount();
      });
      container.remove();
    },
  };
}

describe("useComposerImages", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("attaches files and deduplicates paths", () => {
    const hook = renderComposerImages({
      activeThreadId: "thread-1",
      activeWorkspaceId: "ws-1",
    });

    act(() => {
      hook.result.attachImages(["/tmp/a.png", "/tmp/spec.md"]);
    });

    expect(hook.result.activeImages).toEqual(["/tmp/a.png", "/tmp/spec.md"]);

    act(() => {
      hook.result.attachImages(["/tmp/spec.md", "/tmp/c.pdf"]);
    });

    expect(hook.result.activeImages).toEqual(["/tmp/a.png", "/tmp/spec.md", "/tmp/c.pdf"]);

    hook.unmount();
  });

  it("removes images and clears empty drafts", () => {
    const hook = renderComposerImages({
      activeThreadId: "thread-2",
      activeWorkspaceId: "ws-1",
    });

    act(() => {
      hook.result.attachImages(["/tmp/a.png", "/tmp/b.png"]);
    });

    act(() => {
      hook.result.removeImage("/tmp/a.png");
    });

    expect(hook.result.activeImages).toEqual(["/tmp/b.png"]);

    act(() => {
      hook.result.removeImage("/tmp/b.png");
    });

    expect(hook.result.activeImages).toEqual([]);

    hook.unmount();
  });

  it("switches drafts between thread and workspace", () => {
    const hook = renderComposerImages({
      activeThreadId: "thread-1",
      activeWorkspaceId: "ws-1",
    });

    act(() => {
      hook.result.attachImages(["/tmp/a.png"]);
    });
    expect(hook.result.activeImages).toEqual(["/tmp/a.png"]);

    hook.rerender({ activeThreadId: null, activeWorkspaceId: "ws-1" });
    expect(hook.result.activeImages).toEqual([]);

    act(() => {
      hook.result.attachImages(["/tmp/b.png"]);
    });
    expect(hook.result.activeImages).toEqual(["/tmp/b.png"]);

    hook.rerender({ activeThreadId: "thread-1", activeWorkspaceId: "ws-1" });
    expect(hook.result.activeImages).toEqual(["/tmp/a.png"]);

    hook.unmount();
  });

  it("picks attachment files through the runtime bridge", async () => {
    vi.mocked(pickAttachmentFiles).mockResolvedValue(["/tmp/brief.pdf", "/tmp/screenshot.png"]);
    const hook = renderComposerImages({
      activeThreadId: "thread-3",
      activeWorkspaceId: "ws-1",
    });

    await act(async () => {
      await hook.result.pickImages();
    });

    expect(pickAttachmentFiles).toHaveBeenCalledTimes(1);
    expect(hook.result.activeImages).toEqual(["/tmp/brief.pdf", "/tmp/screenshot.png"]);

    hook.unmount();
  });
});
