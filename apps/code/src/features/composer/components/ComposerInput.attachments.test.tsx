...
function getAttachmentsTray(container: HTMLElement) {
  return container.querySelector('[data-composer-attachments-tray="true"]');
}
...
describe("Composer attachments integration", () => {
  it("attaches dropped files and dedupes paths", async () => {
    ...
    expect(getAttachmentsTray(harness.container)).toBeTruthy();
    ...
    expect(getAttachmentNames(harness.container)).toEqual(["photo.png", "notes.txt", "second.jpg"]);
  ...
  it("removes attachments and clears drafts", async () => {
    ...
    expect(getAttachmentsTray(harness.container)).toBeNull();
  ...
  it("keeps attachments scoped per thread", async () => {
    ...
    const threadOneImage = new File(["data"], "thread-one-spec.md", {
      type: "text/markdown",
    });
    (threadOneImage as File & { path?: string }).path = "/tmp/thread-one-spec.md";

    await act(async () => {
      dispatchDrop(textarea, [threadOneImage]);
    });

    expect(getAttachmentNames(harness.container)).toEqual(["thread-one-spec.md"]);
    ...
    const threadTwoImage = new File(["data"], "thread-two-log.txt", {
      type: "text/plain",
    });
    (threadTwoImage as File & { path?: string }).path = "/tmp/thread-two-log.txt";

    await act(async () => {
      dispatchDrop(getTextarea(harness.container), [threadTwoImage]);
    });

    expect(getAttachmentNames(harness.container)).toEqual(["thread-two-log.txt"]);
  ...
  it("keeps draft attachments scoped per workspace when no thread is active", async () => {
    ...
    const draftImage = new File(["data"], "draft-plan.pdf", {
      type: "application/pdf",
    });
    (draftImage as File & { path?: string }).path = "/tmp/draft-plan.pdf";

    await act(async () => {
      dispatchDrop(textarea, [draftImage]);
    });

    expect(getAttachmentNames(harness.container)).toEqual(["draft-plan.pdf"]);
