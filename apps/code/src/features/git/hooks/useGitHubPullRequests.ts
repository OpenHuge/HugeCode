...
import { getGitHubPullRequests } from "../../../application/runtime/ports/git";
...
export function useGitHubPullRequests(activeWorkspace: WorkspaceInfo | null, enabled: boolean) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (requestIdRef.current !== requestId || workspaceIdRef.current !== workspaceId) {
        return;
      }
      setState({
      ...
      logger.error("Failed to load GitHub pull requests", error);
      if (requestIdRef.current !== requestId || workspaceIdRef.current !== workspaceId) {
        return;
      }
      setState({
