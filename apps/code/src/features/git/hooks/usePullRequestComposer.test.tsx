...
describe("usePullRequestComposer", () => {
  it("prefills composer and switches to PR diff view", () => {
    ...
    expect(options.setActiveTab).toHaveBeenCalledWith("review");
  });
  ...
  it("creates a new thread and sends PR prompt when in PR mode", async () => {
    ...
    expect(buildPullRequestPrompt).toHaveBeenCalledWith(pullRequest, diffs, "Question?");
    expect(options.startThreadForWorkspace).toHaveBeenCalledWith(disconnectedWorkspace.id, {
      activate: false,
    });
  ...
  it("routes slash commands to the normal composer handler in PR mode", async () => {
    ...
    await act(async () => {
      await result.current.handleComposerSend("/compact", []);
    });

    expect(options.handleSend).toHaveBeenCalledWith("/compact", []);
    expect(options.startThreadForWorkspace).not.toHaveBeenCalled();
    expect(options.sendUserMessageToThread).not.toHaveBeenCalled();
  });

  it("routes /compact to the normal composer handler in PR mode", async () => {
    const options = makeOptions({ selectedPullRequest: pullRequest });
    const { result } = renderHook(() => usePullRequestComposer(options));
  ...
  it("routes slash commands through the queue handler when queue is chosen in PR mode", async () => {
    const options = makeOptions({ selectedPullRequest: pullRequest });
    const { result } = renderHook(() => usePullRequestComposer(options));
