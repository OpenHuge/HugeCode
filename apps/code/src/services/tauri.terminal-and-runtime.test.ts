...
describe("tauri invoke wrappers", () => {
  ...
  it("routes runtime job and approval wrappers through runtime client", async () => {
    ...
    const runtimeRunGetV2Mock = vi.fn().mockResolvedValue({
      run: {
        taskId: "task-1",
        workspaceId: "ws-4",
        threadId: "thread-1",
    ...
    const runtimeRunSubscribeV2Mock = vi.fn().mockResolvedValue({
      ...
      missionRun: {
        ...
        state: "running",
        title: "Compile runtime parity",
        summary: "Runtime subscription snapshot",
        startedAt: 110,
        updatedAt: 120,
        routing: {
          backendId: "backend-a",
    ...
    const getRequest = { jobId: "task-1" } as const;
    ...
    await expect(getRuntimeJob(getRequest)).resolves.toEqual({
      id: "task-1",
      workspaceId: "ws-4",
      threadId: "thread-1",
      title: "Compile runtime parity",
    ...
    await expect(subscribeRuntimeJob(subscribeRequest)).resolves.toEqual({
      id: "task-1",
      workspaceId: "ws-4",
      threadId: "thread-1",
      title: "Compile runtime parity",
