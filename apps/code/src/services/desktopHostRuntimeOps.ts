...
export async function isMobileRuntime(): Promise<boolean> {
  return invokeDesktopCommand<boolean>("is_mobile_runtime");
}

export async function orbitConnectTest(): Promise<OrbitConnectTestResult> {
  return invokeDesktopCommand<OrbitConnectTestResult>("orbit_connect_test");
}

export async function orbitSignInStart(): Promise<OrbitDeviceCodeStart> {
  return invokeDesktopCommand<OrbitDeviceCodeStart>("orbit_sign_in_start");
}

export async function orbitSignInPoll(deviceCode: string): Promise<OrbitSignInPollResult> {
  return invokeDesktopCommand<OrbitSignInPollResult>("orbit_sign_in_poll", { deviceCode });
}

export async function orbitSignOut(): Promise<OrbitSignOutResult> {
  return invokeDesktopCommand<OrbitSignOutResult>("orbit_sign_out");
}

export async function orbitRunnerStart(): Promise<OrbitRunnerStatus> {
  return invokeDesktopCommand<OrbitRunnerStatus>("orbit_runner_start");
}

export async function orbitRunnerStop(): Promise<OrbitRunnerStatus> {
  return invokeDesktopCommand<OrbitRunnerStatus>("orbit_runner_stop");
}

export async function orbitRunnerStatus(): Promise<OrbitRunnerStatus> {
  return invokeDesktopCommand<OrbitRunnerStatus>("orbit_runner_status");
}

export async function tailscaleStatus(): Promise<TailscaleStatus> {
  return invokeDesktopCommand<TailscaleStatus>("tailscale_status");
}

export async function tailscaleDaemonCommandPreview(): Promise<TailscaleDaemonCommandPreview> {
  return invokeDesktopCommand<TailscaleDaemonCommandPreview>("tailscale_daemon_command_preview");
}

export async function tailscaleDaemonStart(): Promise<TcpDaemonStatus> {
  return invokeDesktopCommand<TcpDaemonStatus>("tailscale_daemon_start");
}

export async function tailscaleDaemonStop(): Promise<TcpDaemonStatus> {
  return invokeDesktopCommand<TcpDaemonStatus>("tailscale_daemon_stop");
}

export async function tailscaleDaemonStatus(): Promise<TcpDaemonStatus> {
  return invokeDesktopCommand<TcpDaemonStatus>("tailscale_daemon_status");
}

export async function netbirdStatus(): Promise<NetbirdStatus> {
  return invokeDesktopCommand<NetbirdStatus>("netbird_status");
}

export async function netbirdDaemonCommandPreview(): Promise<NetbirdDaemonCommandPreview> {
  return invokeDesktopCommand<NetbirdDaemonCommandPreview>("netbird_daemon_command_preview");
}

export async function getBackendPoolBootstrapPreview(): Promise<BackendPoolBootstrapPreview> {
  return invokeDesktopCommand<BackendPoolBootstrapPreview>("backend_pool_bootstrap_preview");
}
...
export async function runBackendPoolOnboardingPreflight(
  ...
  return invokeDesktopCommand<BackendPoolOnboardingPreflight>("backend_pool_onboarding_preflight", {
    input,
  });
}

export async function getBackendPoolDiagnostics(): Promise<BackendPoolDiagnostics> {
  return invokeDesktopCommand<BackendPoolDiagnostics>("backend_pool_diagnostics");
}
