...
| `code_kernel_job_cancel_v3`, callback register/remove                                 | runtime contract, low-level client extensions, Rust dispatch                                                                                                                                                                                                                                                                                                  | Kernel-job compatibility control residues         | Compat only      | Kept only because `cancel` and callback delivery do not yet have v2-native replacements                               | runtime contract + runtime client + runtime service   | Missing v2 cancel/list or callback replacement |
...
- Compatibility RPCs may stay in the runtime contract only where no v2-native
  replacement exists, and product code must not depend on them as the
  implementation base.
...
- Remaining kernel-job compat residue:
  owner = runtime contract + runtime client
  exit trigger = v2-native cancel/list and callback delivery replacements land
  delete window = first release after downstream parity is green
...
- `runtimeControl.startTask` call sites launch runs without touching deleted
  kernel-job v3 lifecycle RPCs.
