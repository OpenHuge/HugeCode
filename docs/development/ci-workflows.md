...
- PR-triggered Tauri desktop builds should stay host-owned in merge-queue mode: wake them for `apps/code-tauri/**` and desktop-workflow plumbing, not for generic `apps/code` frontend churn. Broader desktop coverage still belongs on `push` to `main`.
